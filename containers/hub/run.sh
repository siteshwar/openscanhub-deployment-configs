...
git clone https://github.com/openscanhub/openscanhub.git /src
git clone https://github.com/release-engineering/kobo.git /src/kobo

# Hostname is different while deploying through helm charts
sed -i "s/'db'/'osh-db'/g" osh/hub/settings_local.py

# collect static files from Django itself
osh/hub/manage.py collectstatic --noinput

for _ in $(seq 100); do
    pg_isready -h osh-db && break
    sleep 0.5
done
